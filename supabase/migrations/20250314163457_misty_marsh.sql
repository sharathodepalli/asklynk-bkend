...
WITH CHECK (
  ...
  (
    ...
    OR
    -- New anonymous thread
    (type = 'anonymous' AND anonymous_thread_id IS NOT NULL)
    OR
    -- Reply to existing anonymous thread
    (
      type = 'anonymous'
      AND
      EXISTS (
